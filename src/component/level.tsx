...
const heavenGeometry = new THREE.BoxGeometry(1, 4, 3);
...
const obstacleMaterial = new THREE.MeshStandardMaterial({ color: '#f5f5dc', metalness: 0, roughness: 1 })
const wallMaterial = new THREE.MeshStandardMaterial({ color: '#f5f5dc', metalness: 0, roughness: 0 })
...
export function BlockStaris({ position = [0, 0, 0 ] }) {
  const obstacle = useRef();
...
export function BlockEnd({ position = [0, 0, 0] }) {
  const hamburger = useGLTF("./hamburger.glb");
    ...
    const gate = useGLTF('https://vazxmixjsiawhamofees.supabase.co/storage/v1/object/public/models/heart/model.gltf')
    ...
    <>
      ...
      <group position={position}>
        <RigidBody
          ...
          friction={0}
        >
          <primitive object={gate.scene} position={[0, 1, 0]} scale={1.5} />
        </RigidBody>
        <mesh
          geometry={boxGeometry}
